...
bool IMU::init(){
  ...
  if (c != 0x71){ // WHO_AM_I should always be 0x68
    return false; // init fails if communication doesn't happen
  }
  ...
  update(); // Make sure variables get initialized
  return true;
}

void IMU::selftest(float &xaccel, float &yaccel, float &zaccel,
                  float &xgyro, float &ygyro, float &zgyro){
  // Trim within x% of factory value
...
float IMU::getYaw(){
  yaw = atan2(2.0f * (qx*qy + q0*qz),
              q0*q0 + qx*qx - qy*qy - qz*qz);
  yaw *= RAD_TO_DEG;
  yaw += MAGNETIC_INCLINATION;
  return yaw;
}

float IMU::getPitch(){
  pitch = -asin(2.0f * (qx*qz - q0*qy));
  pitch *= RAD_TO_DEG;
  return pitch;
}

float IMU::getRoll(){
  roll  = atan2(2.0f * (q0*qx + qy*qz),
                q0*q0 - qx*qx - qy*qy + qz*qz);
  roll *= RAD_TO_DEG;
  return roll;
}
